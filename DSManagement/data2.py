...
class Atributo:
    ...
    def __init__(self, nombre, valores, cortes=None):
        ...
        if all(isinstance(i, (bool, np.bool_)) for i in valores):  # Check if all values are logical (boolean)
            self.tipo = "logical"
        elif ut.isValidNumericVector(valores):  # Check if values are numeric
            self.tipo = "numeric"
        elif ut.isValidCategoricalVector(valores):  # Check if values are categorical
            self.tipo = "character"
        else:
            print(type(valores))
...
class Dataset:
    ...
    def filter_metrics(self, metric_name, operator, value):
        ...
        self.atributos = valid_attributes  # Update the dataset's attributes
        return self
